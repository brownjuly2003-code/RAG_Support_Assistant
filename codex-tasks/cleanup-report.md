...
- `ingestion/` vs `ingestions/`: `ingestion/__init__.py:4-12` — живой пакет ingestion; `ingestions/__init__.py:1-6` описывает "Интеграции" и не имеет consumers (`rg \bingestions\b` по `*.py` вернул 0 hits). Статус: stray typo-package/leftover rename, вероятно вместо `integrations/`. Действие: `delete` `ingestions/` или `rename` в `integrations/` после переноса `bitrix.py`/`mock_inbox.py`.

## 3. Root modules — task-120 dedup aftermath
| Module | Status | Counterpart? | Imported from? | Action |
|--------|--------|--------------|----------------|--------|
| `prompts.py` | корректный deprecation shim (`prompts.py:5-11`) | `agent/prompts.py` ✅ | только fallback-блок `agent/graph.py:64-79` | `keep` временно |
| `state.py` | корректный deprecation shim (`state.py:5-11`) | `agent/state.py` ✅ | только fallback-блок `agent/graph.py:64-79` | `keep` временно |
| `graph.py` | корректный deprecation shim (`graph.py:6-12`) | `agent/graph.py` ✅ | `tests/test_ollama_timeout.py:10,29,51` | `keep`, затем перевести тест на `agent.graph` и `delete` shim |
| `bitrix.py` | реальная реализация в root; docstring говорит `integrations/bitrix.py` (`bitrix.py:1-2`) | `integrations/bitrix.py` / `channels/bitrix.py` ❌ | `mock_inbox.py:54` | `move` в пакет интеграций, затем `shim` или `delete` |
...
| `seed_docs.py` | mislocated script; шапка и runbook говорят `demo/seed_docs.py` (`seed_docs.py:1,15-17`) | `demo/seed_docs.py` ❌ | active consumers не найдены; `archive/legacy-tests/test_retrieval.py:30` ожидает `demo.seed_docs` | `move` в `demo/seed_docs.py` |
...
| `mock_inbox.py` | реальная реализация в root; docstring говорит `integrations/mock_inbox.py` (`mock_inbox.py:2`) | `integrations/mock_inbox.py` / `channels/mock_inbox.py` ❌ | `agent/graph.py:106`, `tests/test_mock_inbox_import.py:2` | `move` вместе с `bitrix.py`, затем `shim` или `delete` |
...
### Missing deps
- `requests` — импортируется в `bitrix.py:42`, но отсутствует и в `requirements.txt`, и в `pyproject.toml`. Действие: `add dep to requirements.txt` и, если проект остаётся dual-manifest, синхронно `add dep to pyproject.toml`.
...
Grep по коду (кроме shim-файлов самих): `from prompts import`, `from state import`, `import prompts`, `import state`.

- Найден только fallback-блок в `agent/graph.py:64-79`.
- Прямых внутренних consumers root-level `prompts.py` и `state.py` в обычном кодовом пути не найдено.
- Статус: почти clean; внутренние импорты уже в основном переведены на `agent.*`.
- Действие: если backward compatibility для root-style execution больше не нужна, `remove fallback block` из `agent/graph.py`, затем на следующем шаге решить судьбу shim-файлов.
...
- Duplicate dirs: 2
- Root modules reviewed: 10; move/shim follow-up нужен для 8, временно корректных shim-only — 2
- Alembic conflicts: 0
- Missing deps: 1 (`requests`); Unused deps: 0 proven; manifest drift: `pyproject.toml` не отражает runtime deps вообще
...
- Deprecated-import violations: 1 fallback block (`agent/graph.py`)
