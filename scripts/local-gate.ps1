...
Add-Gate $gates "mypy strict scope" "python" @("-m", "mypy", "auth", "db/models.py", "db/engine.py", "llm/providers/", "config/settings.py", "agent/state.py", "agent/prompts.py", "agent/prompt_registry.py", "agent/tools.py", "agent/graph.py", "--no-incremental", "--show-error-codes")
...
if ($requirementsTouched) {
    Add-Gate $gates "pip-audit" "pip-audit" @("--strict", "--disable-pip", "--require-hashes", "-r", "requirements.lock")
}
...
if ($List -or $DryRun) {
    ...
    if (-not $requirementsTouched) {
        Write-Host "- pip-audit --strict --disable-pip --require-hashes -r requirements.lock (skipped: lock files unchanged)"
    }
    if ($DryRun) {
