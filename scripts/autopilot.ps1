...
function Invoke-Gates {
    ...
    if (Test-PythonModule "mypy") {
        Invoke-Checked "mypy strict scope 1" "python" @("-m", "mypy", "auth", "db", "llm/providers/", "config/settings.py", "agent/state.py", "agent/prompts.py", "agent/prompt_registry.py", "agent/tools.py", "agent/graph.py", "tasks", "utils", "monitoring", "channels", "--no-incremental", "--show-error-codes")
