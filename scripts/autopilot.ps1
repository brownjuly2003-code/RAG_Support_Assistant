...
function Invoke-Gates {
    ...
    if (Test-PythonModule "mypy") {
        ...
        Invoke-Checked "mypy api.app" "python" @("-m", "mypy", "api/app.py", "api/_shared.py", "api/correlation.py", "api/rate_limit.py", "api/routers", "vectordb", "--no-incremental", "--follow-imports=skip", "--show-error-codes")
    }
    else {
