...
> **SUPERSEDED — этот план не отражает текущее состояние проекта.**
>
> Документ фиксирует MVP→9.0 видение на момент написания; целевой стек ниже
> (React/Next.js SPA, Celery) разошёлся с тем, что реализовано по факту
> (FastAPI + LangGraph + static UI в `static/*.html`). С тех пор в проект
> добавлены JWT/RBAC/SSO-аутентификация, multi-tenancy, Postgres/Redis-backing,
> CI eval- и coverage-гейты, Helm-деплой и другое — иным путём и в ином
> порядке, чем предполагают фазы ниже. Чекбоксы `[ ]`/`[x]` в этом файле **не
> актуализированы построчно** (подтверждение каждого по коду/тестам не
> выполнялось) — не читать их как текущий статус реализации.
>
> Источник актуального состояния:
> - `audit_fable_18_07_26.md` — последний доказательный аудит (18.07.26),
>   сверен с кодом/CI/тестами вживую.
> - `audit_grok_16_07_26.md` — предыдущий доказательный аудит (16.07.26).
> - `AGENT_STATE.md` — журнал фактически выполненной работы по датам.
> - `docs/audits/` — архив более ранних аудитов.
>
> Ниже сохранено как историческая фиксация первоначального скоупа, не как
> актуальный чек-лист задач.
