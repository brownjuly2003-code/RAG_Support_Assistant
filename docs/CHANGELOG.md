...
`include_router` больше не переписывает вложенные маршруты в плоские
префиксованные копии, лист хранит только свой относительный путь.

- **`api/app.py`:** `_extract_route_template` возвращал `route.path_format`
  как есть, поэтому под 0.138 лейбл `endpoint` у **всех** HTTP-метрик менялся
  с `/api/health/live` на `/health/live`, а одноимённые маршруты разных
  роутеров схлопывались в одну серию. Новый `_route_mount_prefix`
  восстанавливает префикс из запроса — **без ветвления по версии**: на ≤0.137
  поправка пустая и результат прежний.
- **`tests/_route_introspection.py` (новый):** общий обход маршрутов для
  тестов владельца. На 0.138 — публичная `fastapi.routing.iter_route_contexts`,
  на ≤0.137 — плоский обход `app.routes`. Внутренности обёртки не трогаем:
  опора на них и создала эту хрупкость.
- **Регрессия закреплена:** `test_extract_route_template_keeps_router_prefix`
  гоняет настоящее приложение с `include_router(prefix=...)`. Существующий
