...
- **Coverage gate 70%** — `[tool.coverage.report] fail_under = 70` в `pyproject.toml`, но реальное число не верифицировано (`test_upload_path_bypasses_body_middleware` зависает в shared-state run).
