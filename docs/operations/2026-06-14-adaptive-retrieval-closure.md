...
**Workstream закрыт, Phase-5 отработан в ОБЕИХ формах.** Phase-5 прогнан и как **replace** (factcard вместо D2 →
вредит, Δ FULL −18), и как **augment** (factcard в дополнение к D2 → безопасно, 0 регрессий, но маржинально:
MISS→PART, Δ FULL на needs +0). **Концепт fact-card полностью охарактеризован данными — обе формы NO-SHIP-to-default.**
Даже D2-чанки ∪ карта не покрывают все ключи целевого MISS → он похож на артефакт корпуса/разметки, а не пробел
извлечения. Числа: `docs/operations/2026-06-15-phase5-factcard-delta.md`. Реактивация — только по явному запросу
владельца (дальнейших retrieval-вариаций концепта нет; opt-in lane остаётся доступен).
