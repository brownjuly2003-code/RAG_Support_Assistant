# Agent State — архив сессий 2026-05-01..2026-06-16

> Вынесено из `AGENT_STATE.md` 2026-07-18 (разгрузка файла, задача N5 из
> `audit_fable_18_07_26.md`). Исторический ledger: ничего здесь не
> редактировать. Актуальное состояние — `AGENT_STATE.md` в корне
> (блок «START HERE»).
>
> Содержит: сессионные блоки, датированные 2026-06-16 и раньше, плюс две
> недатированные майские секции — `Last Verified Gates` (сама объявляет себя
> historical verification ledger) и `Next Step` (майский лог коммитов под
> актуальным заголовком). Обе покрывают 2026-05-01..2026-05-30.
>
> Диапазон в имени пересекается с `agent-state-archive-2026-06-02-to-06-05.md`
> — это не дубль: тот архив вынесен раньше (2026-06-11) и содержит другие
> блоки. Пересечение только в датах, не в содержимом.

Блоки перенесены без изменений, 1:1.
...
>
> **🏁 Workstream adaptive-retrieval ЗАКРЫТ ОКОНЧАТЕЛЬНО, обоснован прогнанной Phase-5-дельтой в ОБЕИХ формах.** Под «решай сам» проверил и вторую форму концепта — **augment** (`--mode augment`, коммит `e31f6e4`: needs-арм = D2-кандидаты ∪ factcard, регрессия невозможна по построению): **безопасно (0 регрессий), но маржинально** — единственный MISS лишь MISS→PART, Δ FULL на needs-срезе +0 (даже D2∪карта не покрывают все ключи целевого MISS → артефакт корпуса/разметки). → **NO-SHIP-to-default и для augment.** Концепт fact-card полностью охарактеризован: replace вредит (−18), augment безопасен но бесполезен. Дальнейших retrieval-вариаций НЕТ (тюнинг k/window не создаст отсутствующих ключей). Opt-in lane (`RAG_RETRIEVAL_STRATEGY=factcard`) остаётся как был. **Backlog проекта для автономной работы ПУСТ** (type-hardening исчерпан, Fable-hardening пуст, adaptive-retrieval закрыт обеими формами).
