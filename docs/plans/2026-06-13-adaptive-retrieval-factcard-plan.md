...
> Статус: **ЗАКРЫТ 2026-06-14; Phase-5 ПРОГНАН 2026-06-15.** Buildable lane (F1–F4) + R1 — DONE,
> зашиплены **opt-in** (`RAG_RETRIEVAL_STRATEGY=factcard`). Phase 3 (авто-роутинг в дефолт) / R2 /
> Phase 4 — **NO-SHIP-to-default**, теперь **обосновано прогнанной Phase-5-дельтой** (композит Δ FULL −18
> на потолке роутера, 19 регрессий на needs-срезе). Решение: `docs/operations/2026-06-14-adaptive-retrieval-closure.md`;
> числа Phase-5: `docs/operations/2026-06-15-phase5-factcard-delta.md`.
...
- [~] **R2: NO-SHIP-to-default (закрыто 2026-06-14).** Врезка классификатора перед `_select_retrieval_strategy` НЕ делается в дефолт: `model_routing_enabled=false` → текущей per-query LLM-стоимости нет → экономия R1 потенциальная, флип добавил бы риск мисроутинга без доказанной выгоды; route-gold *выведен* из query_class, не из измеренного retrieval-выигрыша. **Phase-5 (прогнан 2026-06-15) подтвердил NO-SHIP** для factcard-замены (Δ FULL −18); router-трек = про cost, не recall — отдельно, но в дефолт без явного запроса не идёт. Код R1-классификатора готов к врезке. Обоснование: closure-doc.
...
- [~] **T3.1: NO-SHIP-to-default (закрыто 2026-06-14, подтверждено Phase-5 2026-06-15).** Авто-маршрутизация needs_factcard→factcard в дефолт НЕ включается: Phase-5-дельта показала, что factcard как замена D2 на needs-срезе **регрессирует** (Δ FULL −18, 19 регрессий / 1 улучшение). Шов готов (F4), `RAG_RETRIEVAL_STRATEGY=factcard` даёт ручной heavy-lane. Обоснование: closure-doc + `docs/operations/2026-06-15-phase5-factcard-delta.md`.

### Phase 4 — Эскалация (cascade) + калибровка confidence
- [~] **T4.1/T4.2: NO-SHIP-to-default (закрыто 2026-06-14, подтверждено Phase-5 2026-06-15).** Каскад + AUROC-калибровка имеют смысл только при включённом авто-роутинге; Phase-5 показал NO-SHIP для самой factcard-замены (см. T5.1), так что эскалация поверх неё не строится. Отложены вместе с workstream.

### Phase 5 — Верификация (LAST)
- [x] **T5.1: ПРОГНАНА 2026-06-15 → NO-SHIP по данным.** Офлайн-дельта D2 vs D2+factcard на 100 curated-кейсах исполнена на Mac (harness `scripts/ab_remote_contextual.py` + `scripts/phase5_factcard_delta.py`, скорер `_kw_status`; CPU-embed как обход MPS-OOM). **РЕЗ:** на потолке идеального gold-роутера композит = FULL 79 vs D2 97 (**Δ FULL −18**); на needs-срезе (44) D2 42/44 vs factcard 24/44 — **1 улучшение / 19 регрессий**; целевой `customs-clearance-fields` лишь **MISS→PART** (не закрыт до FULL). Причина — дистиллированные карты теряют точные `answer_contains`-формы, которые D2-чанки несут дословно. → **NO-SHIP-to-default подтверждён ДАННЫМИ.** (Прежняя пометка «НЕ исполнима автономно / harness отсутствует» была НЕВЕРНА.) Числа: `docs/operations/2026-06-15-phase5-factcard-delta.md`.

## Done When
- [x] Роутер выбирает спец-lane только на нужном классе; `customs-clearance-fields` закрыт **или** явно зафиксирован → **явно зафиксирован**: карта ретривится топом (F1–F3), но Phase-5 показал лишь MISS→PART на keyword-метрике; авто-роутинг к нему — NO-SHIP по данным.
