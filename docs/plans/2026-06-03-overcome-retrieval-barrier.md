...
> default off) + `ParentDocumentStore` подключён (`RAG_PARENT_CHILD`). Единственный пробел был
> в no-LLM fallback заголовка — не нёс heading-path. Починено + unit-тест. **Следующий шаг = Phase 1.**
