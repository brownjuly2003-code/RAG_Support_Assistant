...
5. **F-4** — кэш compiled graph + provider runtime. — **✅ СДЕЛАНО (сессия 3, `63a3ee4`)** по спеке `codex-tasks/Archive/task-F4-cache-runtime-graph.md`, все 4 ловушки закрыты: (1) `last_response` → `threading.local`; (2) `_enforce_daily_cost_limit` гоняется на КАЖДЫЙ вызов, включая cache hit (тест-пин); (3) id()-ключи graph-кэша пришпилены strong-ссылками в значениях + LRU cap 16; (4) провайдеры аудированы — per-call HTTP, без разделяемого мутабельного состояния. Тесты `tests/test_provider_runtime_cache.py` 6/6, acceptance-suites 82+25 passed.
...
7. **§3** — решение по multi-worker инварианту (документ или Redis-сессии) — до любого масштабирования реплик. — **✅ СДЕЛАНО** (`d805292`, опция 1): дефолты на 1 worker / 1 replica (`Dockerfile`, helm), claim в `_base_trace.py` починен, startup-warning `WEB_CONCURRENCY>1`, README «Deployment topology». Redis-вынос session/pending-action — отдельный большой цикл при реальной потребности в масштабировании.
8. **F-13, F-14, F-15, F-16** — гигиенический спринт без изменения поведения. — **✅ ЗАКРЫТ (сессия 3)**: F-13 `ebf50a6` (все env-поля Settings на `default_factory` + ast-guard-тест); F-15 `1e0384f` (`inspect.signature` убран из hot path conversation/app, контракты зафиксированы, тест-фейки приведены к реальным сигнатурам) + сопутствующий фикс F-3-регрессии `6326fdc` (quality_source="llm" только при реально распарсенном скоре); F-16 `09a81ce` (audits → `docs/audits/`, stale session-файлы → `docs/sessions/`, AGENT_STATE 1184→659 строк, скриншоты/.coverage удалены). F-14: корневой `cache.py` заархивирован ранее (`84de8e2`); двойники путей `_base_manager._project_root`/warm-cache keying — остаток, не блокер.
