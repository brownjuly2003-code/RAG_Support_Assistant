...
5. **F-4** — кэш compiled graph + provider runtime. — **ДЕ-СКОУПЛЕНО → спека `codex-tasks/task-F4-cache-runtime-graph.md`** (оптимизация, не баг). При разборе найдена 4-я ловушка помимо `last_response`: `build_provider_runtime` → `_enforce_daily_cost_limit` per-request, наивный кэш молча отключит `DAILY_COST_LIMIT_USD` (money-safety). Concurrency+money-sensitive → Codex/отдельный заход.
