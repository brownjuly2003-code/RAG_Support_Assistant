...
3. **Retrieve 1-3: `retrieve`** — hybrid search against a per-tenant ChromaDB collection.
   ...
   collection (`{prefix}_{tenant_id}` — see `vectordb/manager.py`). Cross-tenant
   data cannot share a collection. Postgres-side queries filter by
   `tenant_id` at the ORM layer.
...
The QA prompt presents each retrieved chunk with a `source=<filename>` tag
(see `agent/prompts.py`), and the model is asked to ground its answer in
those passages. A grounded answer for the bundled E20 case looks roughly
like:
...
The exact wording, the suggested follow-ups, and which `source=` tags appear
all vary by run — the block above is illustrative, but the body of the
answer is constrained to what the `errors_e10_e30.md` entry for E20 actually
...
<CardGrid>
  ...
  <Card title="Multi-tenant by construction">
    ...
    (`{prefix}_{tenant_id}`). Postgres-side queries filter by `tenant_id` at
    the ORM layer. Two tenants never share a vector collection.
  </Card>
